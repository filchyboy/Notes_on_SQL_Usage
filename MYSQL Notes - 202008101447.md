...
`DATETIME` is useful for dates prior to and after 1970 while `TIMESTAMP` doesn't work for dates prior to 1970-01-01. Some useful ways to use these functions for keeping track of edits, etc. `created_at TIMESTAMP DEFAULT NOW()` could be useful when creating a `TABLE` for some data. And then when that same data is updated this would be used to track that change: `changed_at TIMESTAMP DEFAULT NOW() ON UPDATE CURRENT_TIMESTAMP`.
